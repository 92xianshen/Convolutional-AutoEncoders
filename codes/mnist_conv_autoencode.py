...
class FlipBatchIterator(BatchIterator):                    #used in defining the layers
...
mu, sigma = np.mean(X.flatten()), np.std(X.flatten())        #computing mean and variance 


# <codecell>

X_train = X.astype(np.float64)
X_train = (X_train - mu) / sigma                            #data preprocessing of input
X_train = X_train.astype(np.float32)

# we need our target to be 1 dimensional
X_out = X_train.reshape((X_train.shape[0], -1))               #defining our output vector
...
#ae.fit(X_train, X_out)                                       #important command ae.fit that performs training of our model
print
...
print X_pred.shape , X.shape      #50000,28,28    50000,1,28,28
...
encode_layer_index = map(lambda pair : pair[0], ae.layers).index('encode_layer')  #encode layer index is found by this statement using       layer definition defined above

encode_layer = ae.get_all_layers()[encode_layer_index]

def get_output_from_nn(last_layer, X):
    indices = np.arange(128, X.shape[0], 128)                #X.shape = 50000*1*28*28 and X.shape[0] = 50000
    sys.stdout.flush()

    # not splitting into batches can cause a memory error
    X_batches = np.split(X, indices)                        #splitting the X (input) into batches of 128 using indices
    out = []                                                #defining output (out) to be a empty matrix
    for count, X_batch in enumerate(X_batches):
        out.append(layers.helper.get_output(last_layer, X_batch).eval()) #out.append appends all the outputs in a single matrix called "out" also layers.helper.get_output(last_layer............... is predefined and is used to obtain o/p of a specific layer in a model
        sys.stdout.flush()
    return np.vstack(out)


def encode_input(X):
    return get_output_from_nn(encode_layer, X)             #encode layer is assigned to the variable last layer in the function
...
next_layer = ae.get_all_layers()[encode_layer_index + 1]           #for decoding the input,we define newlayers and byforth breaking the original layer setup
...
print X_decoded.shape     #50000*784
